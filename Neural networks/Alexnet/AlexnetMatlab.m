...
for i = 1:numel(layers)
    ...
    bodies{i}(1:size(feature_maps{i}, 1), 1:48) = feature_maps{i}(1:size(feature_maps{i}, 1), 1:48);
    hands{i}(1:size(feature_maps{i}, 1), 1:48) = feature_maps{i}(1:size(feature_maps{i}, 1), 49:96);
    faces{i}(1:size(feature_maps{i}, 1), 1:48) = feature_maps{i}(1:size(feature_maps{i}, 1), 97:144);
    objects{i, 1}(1:size(feature_maps{i}, 1), 1:48) = feature_maps{i}(1:size(feature_maps{i}, 1), 145:192);
    objects{i, 2}(1:size(feature_maps{i}, 1), 1:48) = feature_maps{i}(1:size(feature_maps{i}, 1), 193:240);
    objects{i, 3}(1:size(feature_maps{i}, 1), 1:48) = feature_maps{i}(1:size(feature_maps{i}, 1), 241:288);
    mean_objects = zeros(size(objects{i, 1}, 1), size(objects{i, 1}, 2), 3);
    ...
    corr_body_objects(i, 1) = mean(mean(corr(bodies{i}, mean_objects{i})));
    corr_hand_objects(i, 1) = mean(mean(corr(hands{i}, mean_objects{i})));
    corr_face_objects(i, 1) = mean(mean(corr(faces{i}, mean_objects{i})));

    % Tools
    corr_body_tools(i, 1) = mean(mean(corr(bodies{i}, objects{i, 1})));
    corr_hand_tools(i, 1) = mean(mean(corr(hands{i}, objects{i, 1})));
    corr_face_tools(i, 1) = mean(mean(corr(faces{i}, objects{i, 1})));

    % Man
    corr_body_man(i, 1) = mean(mean(corr(bodies{i}, objects{i, 2})));
    corr_hand_man(i, 1) = mean(mean(corr(hands{i}, objects{i, 2})));
    corr_face_man(i, 1) = mean(mean(corr(faces{i}, objects{i, 2})));

    % Nman
    corr_body_nman(i, 1) = mean(mean(corr(bodies{i}, objects{i, 3})));
    corr_hand_nman(i, 1) = mean(mean(corr(hands{i}, objects{i, 3})));
    corr_face_nman(i, 1) = mean(mean(corr(faces{i}, objects{i, 3})));
